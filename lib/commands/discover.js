 ...
 * dry-run by default (per the CLI command spec).
 */
'use strict';
...
const { readUtf8, walkFiles, dirOf } = require('../core/util');
...
module.exports = {
  ...
  run(argv, ctx) {
    ...
    // --kind wins; otherwise the configured default kinds (config
    // control plane); otherwise the full set.
    const configured =
      ctx.config.discover && Array.isArray(ctx.config.discover.default_kinds) && ctx.config.discover.default_kinds.length
        ? ctx.config.discover.default_kinds
        : KINDS;
    const requested = values['--kind'] ? values['--kind'].split(',').map((s) => s.trim()) : configured;
    const badKind = requested.find((k) => !KINDS.includes(k));
    if (badKind) {
      return { error: `invalid kind: ${badKind} (expected ${KINDS.join('|')})`, exit: 2 };
    }
...
function appendToSection(text, section, value) {
  ...
  if (re.test(text)) {
    const sectionText = text.match(re)[1];
    // Idempotent: never append a value that is already a list item.
    const itemRe = new RegExp(`^\\s*[-*•]\\s*${escapeRe(value)}\\s*$`, 'im');
    if (itemRe.test(sectionText)) return text;
...
function removeFromSection(text, section, value) {
  // Line-precise removal: delete exactly the matching list item inside
  // the target section, leaving every other line untouched (the previous
  // regex could consume the blank line and glue headings together).
  ...
  for (const line of lines) {
    ...
    if (heading) {
      inSection = heading[1].replace(/[*`_]/g, '').trim().toLowerCase() === section.toLowerCase();
      out.push(line);
      continue;
    }
    if (inSection && !removed && /^[-*•]\s*/.test(line) && line.replace(/^[-*•]\s*/, '').trim() === value) {
      removed = true;
      continue;
    }
    out.push(line);
