...
async function runInitContext(ctx, { scope, modelId, supervisor = false, templatePath = null }) {
  ...
  if (!hasReadyProvider && !ctx.opts.json) {
    ...
    console.log('The engine uses AI to fill AGENTS.md contracts with real content,');
    console.log('generate .acc-memory.md knowledge, and detect drift automatically.');
    console.log('Without AI, contracts stay as templates with placeholder items.');
    console.log('');
    const answer = await ask('Set up an AI provider now? [Y/n] ');
    rl.close();
    ...
    if (!answer || answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
      ...
      if (addResult.error) {
        console.log(`Provider setup failed: ${addResult.error}`);
        console.log('Continuing without AI — contracts will be created but not filled.');
      ...
      console.log('');
      console.log('Skipping AI setup — continuing without AI.');
      console.log('Contracts will be created as templates. Run `acc engine --init-context` again');
      console.log('after configuring AI to fill them automatically.');
    }
  }
  ...
  lines.push('');
  // 4. AI results.
  const ai = result.engine.ai;
  if (ai && ai.enabled) {
    if (ai.skipped) {
      lines.push(`4. AI: enabled but waiting (${ai.reason}) — run with --force to trigger now`);
    } else if (ai.errors && ai.errors.length) {
      lines.push(`4. AI: errors — ${ai.errors.join('; ')}`);
    } else if (ai.provider) {
      const written = ai.knowledge_written || 0;
      lines.push(`4. AI: ${ai.provider.provider} / ${ai.provider.model}`);
      if (written) lines.push(`   ${written} knowledge entr${written === 1 ? 'y' : 'ies'} written to .acc-memory.md`);
      for (const r of ai.results) {
        const parts = [];
        if (r.knowledge.length) parts.push(`${r.knowledge.length} knowledge`);
        if (r.drift.length) parts.push(`${r.drift.length} drift`);
      ...
      lines.push('4. AI: no providers available — deterministic only');
    }
  } else {
    lines.push('4. AI: disabled (ai.enabled: false) — deterministic only');
  }
  lines.push('');
  const f = result.fill;
  lines.push(`5. Fill: ${f.total} AGENTS.md file(s) — ${f.draft} draft, ${f.complete} complete, ${f.placeholder_items} placeholder item(s) to replace.`);
  if (f.draft || f.placeholder_items) {
    // Check if AI was available — the message changes based on that.
    const aiWasAvailable = ai && ai.enabled && ai.provider;
    if (aiWasAvailable) {
      lines.push('   Contracts filled by AI. Run `acc engine --supervisor --force` to re-score.');
    } else {
      lines.push('   Contracts are templates with placeholder items. To fill them automatically:');
      lines.push('   1. Configure an AI provider: `acc ai add`');
      lines.push('   2. Run `acc engine --init-context` again — the engine will fill all contracts.');
    }
  }
  lines.push('');
  lines.push('Repository initialized with full ACC context.');
  lines.push(`Snapshot saved (${snapshotId}) — run 'acc rollback' to undo.`);
  return { text: lines.join('\n') + '\n' };
}
...
function runRollback(ctx, values) {
  ...
  if (values['--list']) {
    ...
    for (const s of snapshots) {
      lines.push(`  ${s.id}  ${s.command}  ${s.files.length} file(s)  ${s.created}`);
    }
    return { text: lines.join('\n') + '\n' };
  }
  ...
  const doRollback = () => {
    ...
    for (const f of target.files) {
      const action = f.existed ? 'Restored' : 'Removed';
      lines.push(`  ${action} ${f.path}`);
    }
    lines.push('');
