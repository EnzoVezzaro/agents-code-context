...
async function runInitContext(ctx, { scope, modelId, supervisor = false, templatePath = null }) {
  ...
  // 1c. Check if AI is configured. If not, walk the user through setup
  // so the engine can use the AI phase (contracts + knowledge via AI).
  const providers = providersOf(localCtx.config);
  const aiEnabled = localCtx.config.ai && localCtx.config.ai.enabled;
  const hasReadyProvider = providers.some((p) => p.has_api_key && p.installed && !p.errors.length);

  if (!hasReadyProvider && process.stdin.isTTY && !ctx.opts.json) {
    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
    const ask = (q) => new Promise((r) => rl.question(q, (a) => { rl.close(); r(a.trim()); }));

    console.log('');
    console.log('No AI provider configured. The engine uses AI to fill AGENTS.md contracts,');
    ...
    if (!answer || answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
      ...
      if (addResult.error) {
        ...
        console.log('Continuing without AI — contracts will be created but not filled by AI.');
