 ...
 * is present (unless --exit-zero). See the diagnostic codes spec.
 */
'use strict';

const { check } = require('../core/diagnostics');
const { isError } = require('../core/diagnostics');
...
module.exports = {
  ...
  run(argv, ctx) {
    ...
    if (minSeverity) {
      // Minimum severity to emit: --severity warn keeps error + warn,
