 ...
 * to benchmark and test the framework. The framework never requires it and
 * works without it; this command is a convenience launcher only.
 *
...
module.exports = {
  ...
  run(argv, ctx) {
    ...
    if (!abaEntry) {
      return {
        error:
          'ABA (ACC Battle Arena) not found. Install the acc-battle-arena package (npm install acc-battle-arena) or run it from its own repository: node aba/index.cjs <project>',
        exit: 1,
      };
    }
