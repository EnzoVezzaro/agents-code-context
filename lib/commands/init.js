...
const { resolveRoot, load } = require('../config');
const { agentsMdTemplate, configYaml } = require('../templates');
const { check, isError } = require('../diagnostics');
...
module.exports = {
  ...
  usage: 'acc init [directory] [--force] [--scan] [--no-scan]',
  ...
  async run(argv, ctx) {
    ...
    // 6. Print an AGENTS.md template to stdout if none exists (never writes).
