...
module.exports = {
  ...
  async run(argv, ctx) {
    ...
    // 2. Ensure .gitignore excludes ACC-generated files (append if missing).
    const gitignorePath = path.join(root, '.gitignore');
    let gitignore = fs.existsSync(gitignorePath) ? fs.readFileSync(gitignorePath, 'utf8') : '';
    const gitignoreEntries = [
      { pattern: /^\.acc-memory\.md$/m, entry: '.acc-memory.md' },
      { pattern: /^\.acc\/state\/rollback/m, entry: '.acc/state/rollback/' },
    ];
    let additions = '';
    for (const { pattern, entry } of gitignoreEntries) {
    ...
    if (additions) {
      ...
      fs.writeFileSync(gitignorePath, gitignore + sep + additions);
