...
function buildGraph(root, config) {
  ...
  for (const rel of files) {
    ...
    for (const dir of boundaryDirs) {
      ...
      // Path-like match: the boundary path must appear as its own token.
      // Excluded: inside a larger word, as a method call (a regex
      // `.test(`), as a quoted string value (`'test'`), inside a
      // bracket marker (the impact output's `[test]` tag), as an object
      // key (`{ scripts: ... }`), or as the npm `package.json scripts` /
      // `npm scripts` field — none of those are code references to the
      // boundary.
...
module.exports = { buildGraph, SOURCE_EXTS };
