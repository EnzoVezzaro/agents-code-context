...
function buildGraph(root, config) {
  ...
  for (const rel of files) {
    ...
    for (const dir of boundaryDirs) {
      ...
      // `.test(`), as a quoted string value (`'test'`), or inside a
      // bracket marker (the impact output's `[test]` tag) — none of
      // those are code references to the boundary.
      const re = new RegExp("(?<![.\\w'\"`])" + escapeRe(dir) + "\\b(?![\\]])");
      if (re.test(content)) {
