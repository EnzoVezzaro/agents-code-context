 ...
 *
 * Codes and severities are stable per docs/06-diagnostic-codes.md:
 * renumbering or repurposing a code is forbidden. V1 implements the subset
 * below; the full registry is documented in docs/06.
 */
'use strict';

const fs = require('fs');
const path = require('path');
const { walkFiles, readUtf8, relPath, cmp, isUtf8, dirOf } = require('./util');
...
function check(root, config, meta = {}) {
  ...
  for (const rel of files) {
    ...
    let d = dir;
    for (;;) {
      if (boundaryDirs.has(d)) { hasAncestor = true; break; }
      if (d === '') break;
      d = d.slice(0, d.lastIndexOf('/'));
    }
    if (!hasAncestor) {
  ...
  // Orphaned code (ACC072): source dirs with no enclosing boundary.
  ...
  for (const dir of srcDirs) {
    if (boundaryDirs.has(dir)) continue;
    let d = dir;
    let covered = false;
    while (d.includes('/')) {
      d = d.slice(0, d.lastIndexOf('/'));
      if (boundaryDirs.has(d)) { covered = true; break; }
    }
    if (!covered && !boundaryDirs.has('')) {
