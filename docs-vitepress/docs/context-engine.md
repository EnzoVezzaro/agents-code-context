...
Each carries provenance. Inferred constraints are never emitted here; constraints are declared-only by definition (see [03 — Epistemology](./epistemology.md#2-strict-categorization-of-truth)).
...
- JSON format: a `provenance` object per item (see [07 — JSON Output Schema](./json-schema.md)).
...
See [08 — Memory Semantics](./memory-semantics.md).
