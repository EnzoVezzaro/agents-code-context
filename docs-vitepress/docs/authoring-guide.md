...
Paths are canonical references ([02 §7](./repository-structure.md#7-path-conventions-used-by-this-spec)). ACC resolves them to functionality boundaries. Vague names require fuzzy matching, which is less reliable and produces `ACC012` diagnostics when no matching boundary is found.
...
Declared dependencies are the architectural **intent**. They do not need to match discovered imports exactly — that's the point of [03 — Epistemology §5](./epistemology.md#5-truth-resolution): declared wins; mismatches surface as diagnostics.
...
Inheritance is positional (directory hierarchy), not link-based ([02 §1](./repository-structure.md#1-agentsmd--standard-ecosystem)). Links in prose are for the human/agent reader, not for ACC's graph derivation.
...
ACC does not parse frontmatter. The hard invariant ([01](./philosophy.md#13-the-hard-invariant-technical-restatement)) forbids enforcing proprietary schemas in `AGENTS.md`. Put ownership and dependencies in Markdown sections instead.
