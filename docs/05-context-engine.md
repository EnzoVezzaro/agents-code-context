...
`--depth N` limits **transitive expansion of contract context** — how
far away from `<path>` we pull in `AGENTS.md` contents. The underlying
graph (used by `acc graph`, `acc dependencies --transitive`, `acc impact`)
is always fully derivable. Depth is about **how much context we show
the agent**, not about how much we can compute.
...
- `src/audio/AGENTS.md` (local contract)
- inherited `AGENTS.md` chain (root → `src/audio`)
- direct dependencies' contracts (e.g. `src/database/AGENTS.md`)
...
`acc context <path>` emits five sections (filterable via `--include` /
`--exclude`):
...
## Hierarchy
  ...
  └─ src/             AGENTS.md           Source: src/AGENTS.md
      └─ src/audio/   AGENTS.md           Source: src/audio/AGENTS.md
```
...
- **Parsed structure**: sections ACC heuristically detected (e.g.
  `Purpose`, `Dependencies`, `Ownership`, `Constraints`).
- **Raw text reference**: the file path so an agent can read the raw
  Markdown itself (ACC's heuristic parse is non-authoritative; the raw
  text is the source of truth).

Provenance: declared (`src/audio/AGENTS.md`).
...
Transitive rows carry a `hop` count (1 = direct, 2 = one hop away,
etc.). Transitive expansion stops at `--depth`.
...
Declared:
  → src/database/      hop=0   Source: src/audio/AGENTS.md (Dependencies)
  → src/logging/       hop=0   Source: src/audio/AGENTS.md (Dependencies)

Discovered:
  → src/database/      hop=0   Source: Discovered from Rust imports (src/audio/mod.rs)
...
- inherited (declared in ancestor `AGENTS.md` files that apply to this
  subtree)

Each carries provenance. Inferred constraints are never emitted here;
constraints are declared-only by definition (see
[03 — Epistemology](./03-epistemology.md#2-strict-categorization-of-truth)).
...
- file count
- per-language summary (if a language analyzer is available): number of
  modules, top-level functions/classes, exported symbols — counts only,
  NOT source text.
- if no analyzer is available: file count + total bytes + extension
  histogram.

This section is included by default but can be excluded via
`--exclude implementations`. It NEVER contains source code dumps.
...
- JSON format: a `provenance` object per item (see
  [07 — JSON Output Schema](./07-json-schema.md)).

The context engine MUST refuse to emit a context item without provenance.
This is a hard contract, not a best-effort behavior — it's the only way
an agent (or human) can distinguish declared authority from discovered
observation from inferred suggestion in the output.
...
- JSON output sets `truncated: true` and `truncated_bytes_omitted: <N>`
  at the top level.
...
Restrict output to the named sections. Kinds: `hierarchy`, `contract`,
`dependencies`, `constraints`, `implementations`, `memory`.

(`memory` is excluded by default; use `--include memory` to include the
functionality's `.acc-memory.md` existence + contents.)

### `--exclude <kind[,kind...]>`

Remove sections from the default set. E.g. `--exclude implementations`
for a contracts-only view.

`--include` and `--exclude` are mutually exclusive at the command level
...
- Reproducibility is a core ACC value (see
  [01 — Philosophy §7](./01-philosophy.md)).

Order rules:

1. Sections are always emitted in the order: Hierarchy, Contract,
   Dependencies, Constraints, Implementations, Memory.
...
Source: src/audio/AGENTS.md (parsed; raw file is source of truth)
...
## Constraints
- Must not depend on src/ui.   Source: src/audio/AGENTS.md (Constraints)

## Implementations
Files: 12
Languages:
  rust: 9 files, 41 modules, 156 functions
  toml: 3 files
Source: Discovered from filesystem + rust language analyzer

## Memory
.acc-memory.md present at src/audio/.acc-memory.md
