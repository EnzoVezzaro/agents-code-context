...
`AGENTS.md` is the **primary agent interface**. It follows the existing
hierarchical inheritance convention used by Codex, Claude Code, Cursor,
Copilot, and others:
...
A directory containing an `AGENTS.md` represents a
**"functionality boundary."** One functionality = one local `AGENTS.md`.

### Rules

- Markdown. Plain, human-readable.
- **No proprietary schema, no YAML frontmatter, no decorators, no
  mandatory sections.** A directory's `AGENTS.md` is valid even if it is
 one paragraph of prose.
- ACC reads `AGENTS.md` heuristically — it looks for conventional
  section headings (`Purpose`, `Responsibilities`, `Ownership`,
  `Inputs`, `Outputs`, `Dependencies`, `Constraints`, `Architecture`)
  but **never requires** them.
- A directory with no `AGENTS.md` is not a functionality boundary; it is
  just a directory. It inherits context from the nearest ancestor that
  has one.
...
The `.agents/acc/` directory is the **project-level control plane**
specific to ACC. It is extensible, optional, and versioned (committed to
git).

> **Path convention:** the spec text uses `.agents/.acc/` in prose.
> On disk this is the directory `.agents/acc/` (a single `acc` child of
> `.agents/`). Both spellings refer to the same path.
...
Project-level ACC configuration. Optional — sensible defaults apply when
absent. Keys (all optional):
...
`standards/<name>.md` are project standards referenced from `AGENTS.md`.
Human-readable Markdown. E.g. `standards/architecture.md` defines the
project's architecture expectations, referenced by multiple
`AGENTS.md` files via plain prose ("See `.agents/acc/standards/architecture.md`").
...
- **Created** by an agent (or a human) when it learns something durable
  about a functionality that is not yet in `AGENTS.md`.
- **Read** before modifying a functionality to recover lessons learned.
- **Updated** after successful modification, when new durable knowledge
  is worth keeping.
...
as memory; structured memory uses headings as well-known keys (see
[08 — Memory Semantics](./08-memory-semantics.md)).

### Git

`.acc-memory.md` MUST be listed in `.gitignore`. Memory is local and
agent-specific; committing it would create conflicts across agents and
users. The repository's `AGENTS.md` is the durable, committed
...
- Never modifies `AGENTS.md` except via explicit `acc document` or `acc
  discover --apply` commands that require confirmation.
...
- `.agents/acc/` and `.agents/.acc/` refer to the same path; the
  on-disk canonical form is `.agents/acc/`.
- `AGENTS.md` is always lowercase on disk on case-insensitive filesystems;
  ACC treats `AGENTS.md` case-insensitively when matching, but writes the
  canonical `AGENTS.md` form.
- Paths and names are **canonical references**. ACC avoids arbitrary
  opaque IDs; a functionality is identified by its directory path.
